	...
	for _, tst := range tests {
		ts.T().Run(tst.Name, func(t *testing.T) {
			...
			for _, metrics := range tst.SaveMetrics {
				assert.NoError(SaveMetricsForInterval(ts.RedisPool(), tst.Interval, "metrics_test", metrics))
			}

			metrics, err := GetMetrics(ts.RedisPool(), tst.Interval, "metrics_test", tst.GetStart, tst.GetEnd)
			assert.NoError(err)
